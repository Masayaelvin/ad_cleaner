...
latest_quote = None  # Use None instead of an empty string
...
def random_quote():
    ...
    if cache:
        ...
        if datetime.now() - datetime.fromisoformat(latest_quote["timestamp"]) < timedelta(minutes=3):
            print(f"time difference is {datetime.now() - datetime.fromisoformat(latest_quote["timestamp"])} you have to wait for that amount of time to get a new quote")
